...
private:
	...
	void initVulkan() {
		createInstance();		// Step 1: Establish Vulkan connection
		setupDebugMessenger();	// Step 2: Optional debugging setup 
		createSurface();
		pickPhysicalDevice();	// Step 3: Now we can look for GPUs
		createLogicalDevice();
		createSwapChain();
		createImageViews();
		createRenderPass();
